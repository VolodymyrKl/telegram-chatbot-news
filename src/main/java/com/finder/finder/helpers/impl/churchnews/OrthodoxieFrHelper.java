...
@Component
public class OrthodoxieFrHelper extends AbstractRequestSenderService implements ItemsHandler {

    private static Logger logger = LogManager.getLogger(OrthodoxieFrHelper.class);
    ...
    public List<Item> getItems() {
        Rss rss = null;
        try {
            logger.info("Start getting news from Orthodoxie.com.");
            JAXBContext jaxbContext = JAXBContext.newInstance(Rss.class);
            Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();
            URL url = new URL("https://orthodoxie.com/feed/");
        ...
        if (Objects.nonNull(rss)) {
            ...
            for (Item item : filteredItems) {
                item.setTitle(itemsService.translateItem("fr", "uk", item.getTitle()));
                item.setDescription(null);
//                item.setDescription(itemsService.translateItem("fr", "uk", item.getDescription()));
            }
